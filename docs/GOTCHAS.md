...
- **`market` 형식이 안 맞는 줄을 티커로 받으면 쓰레기가 목록에 섞인다.** `KRW-BTC`를 쪼개는 정규식이
  안 맞을 때 원본 문자열을 그대로 티커로 쓰면, 빗썸이 형식을 바꾸거나 안내용 줄을 끼워 넣은 날
  `NOT-A-MARKET-XX` 같은 게 코인으로 들어온다. `_TICKER_RE`로 한 번 더 거르고, 아니면 버린다.
- **빗썸 호출이 실패했다고 캐시를 비우면 안 된다.** 목록이 비면 이름 자동 채우기가 조용히 멈춰서,
  공지 제목이 `(ETH)`처럼 이름 없이 나간다. `refresh()`는 실패해도 갖고 있던 목록을 그대로 두고
  `status()`에 `stale: true`만 남긴다. 200이지만 빈 배열이 온 경우도 같은 이유로 **실패로 친다**.
- **자동완성 순위는 같은 순위 안에서 짧게 맞은 쪽이 먼저다.** '이더'를 치면 이더리움(ETH)과
  이더리움클래식(ETC)이 둘 다 '앞부분 일치'라, 티커 알파벳 순으로 정렬하면 ETC가 위로 온다.
  (순위, 맞은 이름 길이) 순으로 정렬해야 사람이 기대하는 결과가 나온다.
- **사람이 적은 코인 이름을 덮어쓰지 않는다.** `coins.fill()`은 **비어 있는 쪽만** 채운다.
  빗썸 표기와 일부러 다르게 쓰는 공지가 있고, 확실한 건 '비어 있다'는 사실뿐이다.
- **메모리 캐시라 인스턴스마다 따로 논다.** Render 무료 플랜은 15분 놀면 잠들고, 깨어나면
  캐시가 빈 채로 시작한다(뜨자마자 한 번 받으므로 몇 초면 찬다). `/coins`는 캐시가 비어 있으면
  그 요청에서 한 번 받아 채우므로 첫 호출만 느리다.
- **`/health?deep=true`의 `coins.ok`는 셋이다.** `true`(목록 있음) / `false`(한 번도 못 받음 →
  degraded) / `null`(방금 떠서 아직 안 받아 봄 → degraded로 치지 않음). 첫 갱신 전이라는 이유로
  상태가 degraded로 깜빡이지 않게 `null`을 따로 둔다.
...
- **'토큰이 하나인가'로 쪼개짐을 재면 안 된다**
  → `decompound_mode="mixed"`는 복합명사의 원형과 조각을 둘 다 색인한다('비트코인' → 비트코인·비트·코인).
  조각이 더 있어도 원형이 있으면 검색된다. 토큰 수로 재면 멀쩡한 이름까지 고장으로 잡힌다
  (실측 100개 중 72개가 그렇게 잡혔다 — 재색인할 이유가 없는데 있는 것처럼 보였다).
  → **이름이 토큰 목록에 있는가**로 본다. 없으면 그 이름으로 검색해도 안 걸리는 진짜 고장이다.
- **코인 하나마다 _analyze 를 부르면 수백 개에서 시간이 넘는다**
  → 이름당 두 번(지금 분석기·사전 넣은 분석기) 왕복이라 500개면 1000번이다. 실측으로 타임아웃.
  → 줄바꿈으로 이어 붙여 한 번에 보내고 토큰의 start_offset 으로 어느 이름 것인지 가른다.
  오프셋은 보낸 문자열 전체 기준이라 이 계산이 성립한다.
- **CLI로 인덱스를 만들면 사전이 거의 비어 있다**
  → 코인 목록(`coins.known()`)은 프로세스 메모리 한 벌이다. 서버는 lifespan 루프가 채우지만
  CLI는 그 루프가 없어 `setup-index` 시점에 비어 있다. 그러면 별칭 시드만으로 사전이 만들어져,
  `check-dict`가 "333개 고쳐진다"고 한 것과 딴판인 인덱스가 조용히 나온다.
  → `setup-index`가 먼저 `coins.refresh()`를 부르고, 만들어진 사전 개수를 찍는다.
  그 숫자가 `check-dict`의 `rules`와 비슷한지 눈으로 확인하고 넘어간다.
- **인덱스 이름을 환경변수가 직접 가리키면 사전 자동 갱신이 불가능하다**
  → 새 인덱스로 옮긴 뒤 `NOTICE_INDEX`를 손으로 고쳐야 하는데, 그 한 걸음이 사람 손이라
  나머지를 아무리 자동화해도 이어지지 않는다.
  → 별칭(`NOTICE_ALIAS`, 기본 `notices_live`)을 두고 코드는 그것만 본다(`index_ref`).
  전환은 remove+add를 한 번의 `update_aliases`로 해야 한다 — 따로 부르면 그 사이에 별칭이
  아무 인덱스도 안 가리키는 순간이 생긴다.
- **재색인 도중에 들어온 공지가 사라질 수 있다**
  → `reindex`는 시작 시점의 문서만 옮긴다. 도는 동안 수집이 돌면 그 공지는 새 인덱스에 없고,
  그대로 별칭을 돌리면 방금 들어온 공지가 없어진 것처럼 보인다.
  → 전환 전에 원본과 새 인덱스의 건수를 비교해 다르면 멈춘다. 반쯤 옮겨진 인덱스를
  라이브로 올리느니 다음 기회에 다시 도는 편이 낫다.
...
## 맞춤법 검사(/spellcheck)
- **맞춤법 검사가 사실값을 고침** (헤데라→헤더라, 2026.09.25(금)→(목), 빗썸→빗금)
  → 교정된 본문을 돌려주면 factcheck가 교정 뒤를 다시 보지 않아 그대로 새어 나간다.
  → 고쳐진 본문을 만들지 않는다. 위치가 딸린 제안만 주고, 사실값과 겹치는 제안은 코드가 걸러
  protected 로 내려보낸다. 무엇을 막았는지도 함께 준다(검사기가 헛짚는지 보이게).
- **일반 맞춤법 검사기는 빗썸·에어드랍·스테이킹·코인명을 전부 오류로 뱉는다**
  → 사전에 없는 말이라 지적이 쏟아지고 아무도 안 보게 된다.
  → 지시문에 업계 용어를 적어 두고, 코인 이름은 '이름(티커)' 꼴과 data/coin_aliases.json 으로 지킨다.
